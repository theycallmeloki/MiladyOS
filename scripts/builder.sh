...
docker run -d --name miladyos --privileged --user root --restart=unless-stopped --net=host --env MILADY_ADMIN_ID=milady --env MILADY_ADMIN_PASSWORD=milady -v /var/run/docker.sock:/var/run/docker.sock ogmiladyloki/miladyos
docker ps
